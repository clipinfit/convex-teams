...
Run the backend concurrency proof from `example`:
...
Teams mounts `convex-invite` as its child. The host mounts only teams. See [the example configuration](example/convex/convex.config.ts).
...
Create a `TeamsClient` with your host's generated `components.teams` reference. The client methods accept contexts from host Convex functions. The [host example](example/convex/teams.ts) derives the actor from authentication and checks that the invitation recipient has a verified email.
...
[The delivery example](example/convex/delivery.ts) authenticates the actor in a host action. It calls an internal host mutation to issue the invitation, then sends the token from action memory to the provider.
