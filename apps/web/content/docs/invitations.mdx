...
The published `convex-invite@0.1.0` uses native pagination that Convex rejects inside component mounts. This repository applies a Bun patch. A corrected source version is prepared in [invite PR #1](https://github.com/dciccale/convex-invite/pull/1).

A clean npm consumer cannot depend on the repository patch. A corrected invite release remains a prerequisite for the teams runtime release.
